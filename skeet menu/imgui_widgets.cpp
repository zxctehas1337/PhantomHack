...
void ShadowText(const char* label, ImU32 text_col, ImU32 shadow_col, float shadow_thickness, const ImVec2& pos_text)
{
    ...
    //ImGui::ItemSize(size, -55);

    ImGui::GetWindowDrawList()->AddShadowRect(ImVec2(bb.Min.x + 3, bb.Min.y + (label_size.y / 2)), ImVec2(bb.Max.x - 3, bb.Max.y - (label_size.y / 2)), shadow_col, shadow_thickness, ImVec2(0, 0), 100.f);
    //ImGui::GetWindowDrawList()->AddShadowRect(bb.Min + ImVec2(3, (label_size.y / 2) + 1), bb.Max - ImVec2(3, (label_size.y / 2) - 1), shadow_col, shadow_thickness, ImVec2(0, 0), 100.f);
...
#include "examples/example_win32_directx11/bytes/sex.hpp"
#include <map>
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos + ImVec2(0, 2), pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f - 3));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    {
        if (*v)
            window->DrawList->AddImage(widgets::check_box, pos + ImVec2(2, 0), ImVec2(pos.x + 28, pos.y + 26), ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, 255));
        else
            window->DrawList->AddImage(widgets::check_box, pos + ImVec2(2, 0), ImVec2(pos.x + 28, pos.y + 26), ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, 55));
    }
    const ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 6, check_bb.Min.y + style.FramePadding.y + 3.5f);

    if (is_visible && label_size.x > 0.0f)
    {
        if(*v)
            ShadowText(label, ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 145), 25, label_pos);
        else
            ShadowText(label, ImColor(255, 255, 255, 155), ImColor(255, 255, 255, 0), 25, label_pos);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(28, 3), window->DC.CursorPos + ImVec2(w + 30, label_size.y + style.FramePadding.y * 2.0f + 18));
    ...
    window->DrawList->AddRectFilled(bb.Min + ImVec2(0, 18), ImVec2(value_x2, bb.Max.y), ImColor(57, 57, 57));
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(bb.Min + ImVec2(5, 19), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);

    if (label_size.x > 0)
        ShadowText(label, ImColor(255, 255, 255), ImColor(255, 255, 255, 145), 25, ImVec2(bb.Min.x - 1, bb.Min.y + style.FramePadding.y));
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleVarY(ImGuiStyleVar_WindowPadding, 5); // Horizontally align ourselves with the framed text
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(28, 4), window->DC.CursorPos + ImVec2(w + 52, label_size.y + style.FramePadding.y * 2.0f + 12));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    // Slider behavior
    ImRect grab_bb;
    bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);

    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 20), frame_bb.Max - ImVec2(0, 1), ImColor(57, 57, 57), style.GrabRounding);
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 20), frame_bb.Max - ImVec2(0, 1), ImColor(57, 57, 57), style.GrabRounding);
    ...
    if ((frame_bb.Min.x + it_filing->second.value) < grab_bb.Min.x) {
        it_filing->second.speed = (grab_bb.Min.x - (frame_bb.Min.x + it_filing->second.value)) / 14.f;
        it_filing->second.value += it_filing->second.speed;
    }
    else if ((frame_bb.Min.x + it_filing->second.value) > grab_bb.Min.x) {
        it_filing->second.speed = ((frame_bb.Min.x + it_filing->second.value - grab_bb.Min.x)) / 14.f;
        it_filing->second.value -= it_filing->second.speed;
    }

    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 20), ImVec2(frame_bb.Min.x + it_filing->second.value + 13, grab_bb.Max.y + 1), ImColor(255, 255, 255));
        window->DrawList->AddShadowRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 20), ImVec2(frame_bb.Min.x + it_filing->second.value + 13, grab_bb.Max.y + 1), ImColor(255, 255, 255, 145), 30, ImVec2(0, 0));

        //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 20), grab_bb.Max + ImVec2(2, 1), ImColor(255, 255, 255));
        //window->DrawList->AddShadowRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 20), grab_bb.Max + ImVec2(2, 1), ImColor(255, 255, 255, 145), 30, ImVec2(0, 0));
    }
    ...
    if (format != "")
    {
        ...
        RenderTextClipped(frame_bb.Min + ImVec2(-58 + it_filing->second.value, 0), frame_bb.Max + ImVec2(-58 + it_filing->second.value, 34), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
        window->DrawList->AddShadowRect(frame_bb.Min + ImVec2(8 + it_filing->second.value, 30), frame_bb.Max + ImVec2(-125 + it_filing->second.value, 4), ImColor(255, 255, 255, 85), 20, ImVec2(0, 0));
        ImGui::SetWindowFontScale(1.0f);
    }

    if (label_size.x > 0.0f)
        ShadowText(label, ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 145), 25, ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        ...
        if (hovered)
            RenderFrame(bb.Min, bb.Max, ImColor(52, 52, 52), true, style.FrameRounding);
    }
