...
bool PlayerPawn::GetGrenades()
{
	// Initialize grenades string
	Grenades = "";

	// Get weapon services to access all weapons
	DWORD64 weaponServices;
	if (!memoryManager.ReadMemory(this->Address + Offset.Pawn.m_pWeaponServices, weaponServices) || weaponServices == 0)
		return true;

	// Get weapons array
	DWORD64 weaponsArray;
	if (!memoryManager.ReadMemory(weaponServices + 0x8, weaponsArray) || weaponsArray == 0)
		return true;

	// Get weapons count
	int weaponsCount;
	if (!memoryManager.ReadMemory(weaponServices + 0x10, weaponsCount) || weaponsCount <= 0)
		return true;

	std::vector<std::string> grenadeNames;
	
	// Loop through all weapons to find grenades
	for (int i = 0; i < weaponsCount && i < 64; i++) // Limit to 64 weapons
	{
		DWORD64 weaponPtr;
		if (!memoryManager.ReadMemory(weaponsArray + i * 0x8, weaponPtr) || weaponPtr == 0)
			continue;

		// Get weapon definition index
		DWORD64 weaponIndexAddress = weaponPtr + Offset.EconEntity.AttributeManager +
			Offset.WeaponBaseData.Item + Offset.WeaponBaseData.ItemDefinitionIndex;
		...
		// Check if it's a grenade by weapon ID
		// Common grenade weapon IDs in CS2
		if (weaponIndex == 44 || weaponIndex == 45 || weaponIndex == 46 || 
			weaponIndex == 47 || weaponIndex == 48 || weaponIndex == 49 ||
			weaponIndex == 67 || weaponIndex == 68 || weaponIndex == 69 ||
			weaponIndex == 70 || weaponIndex == 71 || weaponIndex == 72 ||
			weaponIndex == 74 || weaponIndex == 75 || weaponIndex == 76 ||
			weaponIndex == 78 || weaponIndex == 79 || weaponIndex == 80 ||
			weaponIndex == 81 || weaponIndex == 82)
		{
			// Map weapon IDs to grenade types
			std::string grenadeType;
			if (weaponIndex == 44 || weaponIndex == 67) grenadeType = "HE";
			else if (weaponIndex == 45 || weaponIndex == 68) grenadeType = "Flash";
			else if (weaponIndex == 46 || weaponIndex == 69) grenadeType = "Smoke";
			else if (weaponIndex == 47 || weaponIndex == 70) grenadeType = "Molotov";
			else if (weaponIndex == 48 || weaponIndex == 71) grenadeType = "Inc";
			else if (weaponIndex == 49 || weaponIndex == 72) grenadeType = "Decoy";
			else if (weaponIndex >= 74 && weaponIndex <= 82) grenadeType = "Tactical"; // Other tactical grenades
			
			// Check if we already have this type (avoid duplicates)
			if (std::find(grenadeNames.begin(), grenadeNames.end(), grenadeType) == grenadeNames.end())
			{
				grenadeNames.push_back(grenadeType);
			}
		}
	}

	// Create grenades string
	if (!grenadeNames.empty())
	{
		for (size_t i = 0; i < grenadeNames.size(); i++)
		{
			if (i > 0) Grenades += " ";
			Grenades += grenadeNames[i];
		}
	}
